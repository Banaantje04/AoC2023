...
fn solve_part_1() -> isize {
    INPUT.lines().map(|l| {
        let mut sequences: Vec<Vec<isize>> = vec![l.split_whitespace().map(|n| n.parse().unwrap()).collect()];
        while !sequences.last().unwrap().iter().all(|s| *s == 0) {
            sequences.push(sequences.last().unwrap().windows(2).map(|n| n[1] - n[0]).collect());
        }
